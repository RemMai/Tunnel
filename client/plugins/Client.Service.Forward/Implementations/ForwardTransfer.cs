...
namespace client.service.forward.Implementations
{
    ...
    public sealed class ForwardTransfer
    {
            ...
            Common.ForWard.Config config, IForwardProxyPlugin forwardProxyPlugin, IProxyServer proxyServer)
        {
            ...
            forwardProxyPlugin.AfterStart += (port) => StateChanged(port, true);
            forwardProxyPlugin.AfterStop += (port) => StateChanged(port, false);
        }
        ...
        private void StateChanged(ushort port, bool state)
        {
            if (port == 0)
            {
                p2pListens.ForEach(c => { c.Listening = state; });
            }
            else
            {
